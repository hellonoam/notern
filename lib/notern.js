...
Notern.prototype.init = function() {
  ...
  app.configure('production', function() {
	...
	app.get("/user/:userId/notes/:noteId", function(req, res) {
		...
		snc.getNote(noteId, userId, function(oldNoteJson) { 
			res.contentType('application/json');
	...
	app.delete("/user/:userId/notes/:noteId", function(req, res) {
		...
		var deleteNote = new ServerNote(userId, {"noteId" : noteId});
		deleteNote.destory();
	});
...
Notern.prototype.listen = function() {
  ...
  var note = {owner: 100, lastModified: 256, json:{willy: "poo"}};
  db.insertNewNote(note, function(object) { console.log(note.id) });
  db.findUserNotes(100, function(object) { console.log(object) });
};
