...
var uc = new userController(); // userDb and sessionDb
...
Notern.prototype.init = function() {
	...
	app.get("/user/:username/notes", function(req, res) {
		...
		uc.authenticateSession(username, req.cookies.key, function(response) {
			if (response != true)
			...
			snc.findUserNotes(username, since, function(userNotes) {
	...
	app.get("/user/:username/notes/:noteId", function(req, res) {
		...
		uc.authenticateSession(username, req.cookies.key, function(response) {
			if (response != true)
				return  res.send('unauthorized - invalid cookie', { 'Content-Type': 'text/plain' }, 401);
			snc.findSingleNote(noteId, username, function(oldNoteJson) { 
				res.contentType('application/json');
